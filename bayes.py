...
def calcular_parametros(x, y, dim):
    ...
    if dim == 2:
        # Grillas
        b_grid = np.mgrid[7e-1:8.2e-1:100j, 3.2:4.2:100j]
        b0_grid, b1_grid = b_grid
        dA = 1.2e-1 / 100
        dsigma = 1. / 100
        prior_params = [7.5e-1, 1e-1, 3.5, 1.]
    ...
    if dim == 4:
        # Grillas
        b_grid = np.mgrid[3.8e-1:5.8e-1:20j, 7.4:9.4:20j,
                          3.1e-1:5.1e-1:20j, 1.4:3.4:20j]
        b0_grid, b1_grid, b2_grid, b3_grid = b_grid
        # Anchos de bins
        dA1 = 2e-1 / 20
        dA2 = 2e-1 / 20
        ...
        prior_params = [4e-1, 1e-1, 2., 1., 4.5e-1, 1e-1, 8., 1.]
...
def resultados_gauss(x, y, seeds=False):
    ...
    print 'Metodo: curve_fit'
    print 'A1 = ', popt3[0] * 1e-16
    print 'A2 = ', popt3[2] * 1e-16
    print 'sigma1 = ', popt3[1]
    print 'sigma2 = ', popt3[3]
    print 'chi**2 = ', chi(doblegauss, x, y, popt3)
    print 'Metodo: bayes'
    print 'A1 = ', popt4[1] * 1e-16
    print 'A2 = ', popt4[3] * 1e-16
    ...
    # Graficar gaussiana
    p.plot(x, y * 1e-16, color='turquoise', drawstyle='steps-post', lw=2., alpha=0.8)
    p.plot(X, gauss(X, popt1[0], popt1[1]) * 1e-16, 'b', lw=2., alpha=0.8)
    ...
    p.plot(x, y * 1e-16, color='turquoise', drawstyle='steps-post', lw=2., alpha=0.8)
    p.plot(X, doblegauss(X, *popt3) * 1e-16, 'b', lw=2., alpha=0.8)
    p.plot(X, gauss(X, popt3[0], popt3[1]) * 1e-16, 'g--', lw=2., alpha=0.8)
    p.plot(X, gauss(X, popt3[2], popt3[3]) * 1e-16, 'g--', lw=2., alpha=0.8)
    p.axis([6503, 6623, 9e-17, 1.01e-16])
    p.xlabel('Angstrom')
    p.ylabel('erg / s / Hz / cm^2')
    p.grid(True)
    ...
    p.plot(x, y * 1e-16, color='turquoise', drawstyle='steps-post', lw=2., alpha=0.8)
    p.plot(X, gauss(X, popt1[0], popt1[1]) * 1e-16, 'b', lw=2., alpha=0.8)
    p.plot(X, gauss(X, popt2[1], popt2[2]) * 1e-16, 'g', lw=2., alpha=0.8)
    p.axis([6503, 6623, 9e-17, 1.01e-16])
    p.xlabel('Angstrom')
    p.ylabel('erg / s / Hz / cm^2')
    p.grid(True)
    ...
    p.plot(x, y * 1e-16, color='turquoise', drawstyle='steps-post', lw=2., alpha=0.8)
    p.plot(X, doblegauss(X, *popt3) * 1e-16, 'b', lw=2., alpha=0.8)
    p.plot(X, doblegauss(X, popt4[1], popt4[2], popt4[3], popt4[4]) * 1e-16,
           'g', lw=2., alpha=0.8)
    p.axis([6503, 6623, 9e-17, 1.01e-16])
    p.xlabel('Angstrom')
    p.ylabel('erg / s / Hz / cm^2')
...
p.plot(x, y * 1e-16, color='turquoise', drawstyle='steps-post', lw=2., alpha=0.8)
p.plot(x[50], y[50] * 1e-16, '*')
p.plot(x[76], y[76] * 1e-16, '*')
p.axis([6503, 6623, 9e-17, 1.01e-16])
p.xlabel('Angstrom')
p.ylabel('erg / s / Hz / cm^2')
p.show()
...
print 'Desviacion STD fuera de la linea de absorcion = ', error * 1e-16
