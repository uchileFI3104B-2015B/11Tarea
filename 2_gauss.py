...
def prior(beta, params):
    ...
    S = (-1. / 2 * ((beta0 - mu0)**2 / sigma0**2 +
         (beta1 - mu1)**2 / sigma1**2 + (beta2 - mu2)**2 / sigma2**2 +
         (beta3 - mu3)**2 / sigma3**2))
...
def likelihood(beta, data, sigma_datos):
    ...
    try:
        ...
        N = 1
    mu = 6563
    S = np.sum((y - 1e-16 * ESCALA - beta0 * norm(loc=mu, scale=beta1).pdf(x) -
               beta2 * norm(loc=mu, scale=beta3).pdf(x))**2)
...
with basic_model:
    ...
    y_out = (1e-16 * ESCALA - A * 1/(sigma * np.sqrt(2 * np.pi)) *
             T.exp(-0.5 * ((wl-mu)/sigma)**2) -
             A2 * 1/(sigma2 * np.sqrt(2 * np.pi)) *
             T.exp(-0.5 * ((wl-mu)/sigma2)**2))
...
ax1.plot(wl, (1e-16 * ESCALA - float(map_estimate["amplitud"]) *
              norm(loc=6563, scale=float(map_estimate["sigma"])).pdf(wl) -
              float(map_estimate["amplitud 2"]) *
              norm(loc=6563, scale=float(map_estimate["sigma 2"])).pdf(wl)),
         label="fit, gaussiana doble")
...
# los parametros adecuados los podemos estimar de la muestra
# obtenida usando pymc3 o Metropolis.
mu = [map_estimate["amplitud"], map_estimate["sigma"],
      map_estimate["amplitud 2"], map_estimate["sigma 2"]]
algo = 1e-3
sigma = [[algo, 0, 0, 0], [0, algo, 0, 0],
         [0, 0, algo, 0], [0, 0, 0, algo]]
nd_normal = multivariate_normal(mu, sigma)

prior_params = [map_estimate["amplitud"], 1e16 * ESCALA, map_estimate["sigma"],
                100, map_estimate["amplitud 2"],
                1e16 * ESCALA, map_estimate["sigma 2"], 100]
N_sample = 5000
