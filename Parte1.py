# -*- coding: utf-8 -*-
"""
Se modela una línea de absorción de una observación espectroscópica
...
plt.ylabel('Flujo por unidad de freuencia [erg / s / Hz / cm^2 * 10^19]')

beta0_grid, beta1_grid = np.mgrid[6400:6700:201j, 900:1100:201j]
beta_prior_pars = [6563, 58, 1000, 14]
prior_grid = fill_prior(beta0_grid, beta1_grid, beta_prior_pars)

plt.figure(1)
plt.pcolormesh(beta0_grid, beta1_grid, prior_grid, cmap='summer')
...
plt.figure(2)
plt.pcolormesh(beta0_grid, beta1_grid, post_grid, cmap='summer')
plt.xlabel('Longitud de onda w [Angstrom]')
plt.ylabel('Flujo por unidad de freuencia [erg / s / Hz / cm^2 * 10^19]')
ax1, ax2, ax3 = plot_distribution(beta0_grid, beta1_grid,
                                  post_grid, cmap='summer')
ax1.axvline(6675, lw=0.5, color='w')
ax1.axhline(1025, lw=0.5, color='w')
ax2.axvline(6675, lw=1., color='k')
ax3.axhline(1025, lw=1., color='k')
